...
test('files', async () => {
  ...
  const result2 = await agent.post(TEST_URL).attach('file', filePath).attach('file2', filePath);
  expect(result2.status).toEqual(400);
  expect(result2.text).toEqual('"value" must be [image/png]');

  const result3 = await agent.post(TEST_URL).attach('file', filePath).attach('file3', filePath);
  expect(result3.status).toEqual(400);
  expect(result3.text).toEqual('"value" must be [avatar2.jpg]');

  const result4 = await agent.post(TEST_URL).attach('file', filePath).attach('file4', filePath);
  expect(result4.status).toEqual(400);
  expect(result4.text).toEqual('"value" must be [.jpeg]');

  const result5 = await agent.post(TEST_URL).attach('file', filePath).attach('file5', filePath);
  expect(result5.status).toEqual(400);
  expect(result5.text).toEqual('"value" must be [avatar2]');

  const result6 = await agent.post(TEST_URL).attach('file', filePath).attach('file6', filePath);
